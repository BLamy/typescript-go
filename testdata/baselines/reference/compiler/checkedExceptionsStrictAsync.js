...
// A synchronous try/catch does not handle a promise rejection.
function fakeSynchronousHandling() {
    try {
        rejects();
    }
    catch {
...
// Awaiting an ambient promise is catchable, but its catch value is unknown.
async function handledLegacyAwait() {
    try {
        await legacyAsync();
    }
    catch (e) {
...
// Returning an ambient promise requires honest unknown propagation.
async function returnedLegacyPromise() {
    return legacyAsync();
}
function callbackThrows() {
    throw "later";
}
function synchronousCatchCannotHandleCallback() {
    try {
        later(callbackThrows);
    }
    catch {
...
// Handling inside the callback is valid regardless of when it is invoked.
function callbackHandlesItsOwnEffect() {
    later(() => {
        try {
            callbackThrows();
        }
