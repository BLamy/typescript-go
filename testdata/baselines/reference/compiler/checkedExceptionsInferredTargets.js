...
//// [checkedExceptionsInferredTargets.js]
"use strict";
...
f = throwsE2; // error
f = (x) => { if (x)
    throw new E1(); }; // ok: same set
// Same through a function declaration's type.
function named(x) { if (x)
    throw new E1(); }
let h = named;
h = throwsE2; // error
// An initializer whose visible body throws nothing is proven safe, so a
